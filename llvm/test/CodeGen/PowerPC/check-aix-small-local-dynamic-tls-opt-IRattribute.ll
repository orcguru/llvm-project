...
; Check that the aix-small-local-dynamic-tls attribute is not supported on Linux and AIX (32-bit).
; CHECK-NOT-SUPPORTED: The aix-small-local-[exec|dynamic]-tls attribute is only supported on AIX in 64-bit mode.

; Check that the aix-small-local-dynamic-tls attribute is only supported when
...
; aix-small-local-dynamic-tls attribute.
; CHECK-LABEL: testWithIRAttr:
; CHECK:        li r3, 0
; CHECK-NEXT:   blr


attributes #0 = { "target-features"="+aix-small-local-dynamic-tls" }
